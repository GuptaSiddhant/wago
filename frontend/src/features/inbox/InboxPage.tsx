...
import { Send, Inbox as InboxIcon, CheckCheck } from 'lucide-react'
      ...
      >
        ...
        <div
          className={`mt-1 flex items-center justify-end gap-1 text-[10px] ${
            outbound ? 'text-emerald-200/80' : 'text-zinc-500'
          }`}
        >
          <span>{formatTime(message.created)}</span>
          {outbound ? <CheckCheck size={11} /> : null}
        </div>
      </div>
    </li>
  )
...
function Composer({ conversationId, onSent }: { conversationId: string; onSent: () => void }) {
  ...
  const sendMutation = useMutation({
    mutationFn: (body: string) => sendMessage({ conversation_id: conversationId, body }),
    onSuccess: () => {
      setDraft('')
      setError(null)
  ...
  return (
    <form onSubmit={handleSubmit} className="border-t border-zinc-800/80 p-3">
      ...
      <div className="flex items-end gap-2">
        ...
        <Button
          type="submit"
          isDisabled={draft.trim() === '' || sendMutation.isPending}
